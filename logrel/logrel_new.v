...
Definition val_type_naked (T : ty) : (forall T', R T' T -> denv -> Dom) -> denv -> Dom :=
  ...
  | TSel (varF x) => fun _ ρ =>
                       ...
                       | Some D => {{ v | exists vty, vty ∈ D /\ exists (X : Dom), v ∈ X }}
                       | None   => DBot
                       end

  | TMem T1 T2    => fun val_type ρ =>
                      {{ '(vty gamma T) | exists X, (val_type T1 RMem1 ρ) ⊆ X /\ X ⊆ (val_type T2 RMem2 ρ) }}
...
Lemma val_type_extend : forall {T ρ E}, val_type T ρ === val_type T (E :: ρ).
Proof.
  unfold subset.
  ...
  - (* TMem *)
    split; intros; destruct v as [ γ T' t | γ T' ]; prim_unfold_val_type in H; prim_unfold_val_type; auto; intros;
      unfold elem in *; unfold elem2 in *; auto; try destruct H as [X [Helem [T1subX XsubT2]]];
        try destruct H as [X [T1subX XsubT2]]; exists X; repeat split; try assumption;
          try (apply (subset_trans XsubT2); unfold subset; apply (IHT _ RMem2));
            eapply subset_trans; try eassumption; unfold subset; apply (IHT _ RMem1).
Admitted.

Lemma val_type_suffix : forall {T ρ ρ' }, val_type T ρ === val_type T (ρ' ++ ρ).
...
Definition 𝒞𝓉𝓍_Inv (x : id) (T : ty) (E : Dom) : Prop :=
  exists Γ, exists ρ, E = (val_type T ρ)
            /\ 𝒞𝓉𝓍 Γ ρ
            /\ length Γ = length ρ
...
Definition ℰ𝓃𝓋_Inv (x : id) (E : Dom) (v : vl) : Prop :=
  exists ρ, exists γ, v ∈ E
            /\ ℰ𝓃𝓋 ρ γ
            /\ length ρ = length γ
...
Lemma fundamental' :  (forall {Γ t T}, has_type Γ t T -> forall{ρ}, 𝒞𝓉𝓍 Γ ρ -> forall{γ}, ℰ𝓃𝓋 ρ γ -> ⟨ γ , t ⟩ ∈ ℰ (val_type T ρ))
                    /\ (forall {Γ S T}, stp Γ S T      -> forall{ρ}, 𝒞𝓉𝓍 Γ ρ -> forall{γ}, ℰ𝓃𝓋 ρ γ -> (val_type S ρ) ⊆ (val_type T ρ)).
Proof.
  ...
  - (* t_typ *)
    ...
    exists 1. exists (vty γ T). simpl. split. simpl. reflexivity. exists (val_type T ρ).
    apply seteq_refl.
  - (* t_abs *)
    ...
    intros vx vxinT1.
    assert (HextG : 𝒞𝓉𝓍 (T1 :: Γ) ((val_type T1 ρ) :: ρ)). { (* TODO follows from consistent context/environment assumptions *)
             admit.
             }
    assert (Hextg : ℰ𝓃𝓋 ((val_type T1 ρ) :: ρ) (vx :: γ)). { (* TODO follows from consistent context/environment assumptions *)
             admit.
    }
    specialize (IH _ HextG _ Hextg).
    destruct IH as [k [vy [evalvy vyinVtyT2 ] ]].
    exists k. exists vy. split. assumption.
    unfold elem. prim_unfold_val_type.
    replace (open' γ T2) with (open' Γ T2). assumption.
    unfold open'. replace (length γ) with (length Γ). reflexivity.
    apply 𝒞𝓉𝓍_length in HΓρ. rewrite HΓρ. apply ℰ𝓃𝓋_length. auto.
  - (* t_app *)
    ...
    eapply eval_monotone in evalv1. eapply eval_monotone in evalapp. eapply eval_monotone in evalv2. simpl. erewrite evalv2. simpl. erewrite evalv1. erewrite evalapp.
  ...
  - (* t_dapp *)
    ...
    exists (k1 + k2 + k3). exists v3. split.
    destruct k1; destruct k2; destruct k3; try solve [ simpl in *; discriminate].
    eapply eval_monotone in evalv1. eapply eval_monotone in evalapp.
    eapply eval_monotone in evalv2. simpl. erewrite evalv2. simpl. erewrite evalv1.
    erewrite evalapp. reflexivity. lia. lia. lia.
  ...
  - (* stp_mem *)
    ...
    eapply subset_trans. eauto. assumption.
  - (* stp_sel1 *)
    intros Γ x T1 T HT1 HStp IH ρ HΓρ γ Hργ v vinT.
    prim_unfold_val_type.
    assert (Hrho : exists ρ', indexr x ρ = Some (val_type T1 ρ')). { (* TODO follows from consistent context/environment assumptions *)
               admit.
    }
    destruct Hrho as [ρ' Hrho ]. rewrite Hrho.
    specialize (IH ρ HΓρ γ Hργ).
    assert (Hvty : exists vty, indexr x γ = Some vty /\ vty ∈ (val_type T1 ρ')). {
      admit. (* TODO follows from consistent context/environment assumptions *)
    }
    destruct Hvty as [vtype [Hvtyγ vtyinT1]].
    assert (T1ext : val_type T1 ρ' ⊆ val_type T1 ρ). {
      admit. (* from consistent context/environment assumptions & val_type_extend *)
    }
    unfold elem in vtyinT1. assert (vtyProp := vtyinT1).
    apply T1ext in vtyProp. apply IH in vtyProp.
    destruct vtype as [ γ' T'' t | γ' T'' ] eqn:Heqvty; prim_unfold_val_type in vtyProp.
    contradiction. destruct vtyProp as [X [TsubX XsubTop]].
    exists vtype. split. unfold elem. subst. assumption.
    exists X. unfold elem. apply TsubX. assumption.
  - (* stp_sel 2*)
    intros Γ x T1 T HT1 HStp IH ρ HΓρ γ Hργ v vinxType.
    prim_unfold_val_type in vinxType.
    assert (Hrho : exists ρ', indexr x ρ = Some (val_type T1 ρ')). { (* TODO follows from consistent context/environment assumptions *)
               admit.
    }
    destruct Hrho as [ρ' Hrho ]. rewrite Hrho in vinxType.
    specialize (IH ρ HΓρ γ Hργ).
    assert (Hvty : exists vty, indexr x γ = Some vty /\ vty ∈ (val_type T1 ρ')). {
      admit. (* TODO follows from consistent context/environment assumptions *)
    }
    ...
    destruct vtype as [ γ' T'' t | γ' T'' ] eqn:Heqvty; prim_unfold_val_type in vtyProp.
    contradiction. destruct vtyProp as [X [TsubX XsubTop]].
    (* FIXME: here, the X in vinxType is not connected to the other X in assms *)
      admit.
  - (* stp_selx *)
...
Theorem fundamental : forall {Γ t T}, has_type Γ t T -> forall{ρ}, 𝒞𝓉𝓍 Γ ρ -> forall{γ}, ℰ𝓃𝓋 ρ γ -> ⟨ γ , t ⟩ ∈ ℰ (val_type T ρ).
Proof.
  destruct fundamental' as [fund _ ].
  apply fund.
Qed.

Theorem  fundamental_stp : forall {Γ S T}, stp Γ S T -> forall{ρ}, 𝒞𝓉𝓍 Γ ρ -> forall{γ}, ℰ𝓃𝓋 ρ γ -> (val_type S ρ) ⊆ (val_type T ρ).
Proof.
