...
Record TDEnvR (x : id) (Γ : tenv) (T : ty) (ρ : denv) (E : DEntry) : Prop :=
  mkCProp {
      Γ_l             : tenv;
      Γ_r             : tenv;
      ρ_l             : denv;
      ρ_r             : denv;
      length_eq_Γρl   : length Γ_l = length ρ_l;
      length_eq_Γρr   : length Γ_r = length ρ_r;
      length_eq_x_Γρr : x = length Γ_r;
      Γ_decomp        : Γ = Γ_l ++ (T :: Γ_r);
      ρ_decomp        : ρ = ρ_l ++ (E :: ρ_r);
      Γρ_wf           : 𝒞𝓉𝓍 Γ_r ρ_r;
      lookup_ρx       : indexr x ρ = Some E;
      ρx              : E = {| ValF := (val_type T Val ρ_r); TypF := (val_type T Typ ρ_r) |}
    }.
...
Definition lookup_agrees' {A B} (xs : list A) (ys : list B) (P : id -> list A -> A -> list B -> B -> Prop) :=
                  forall {x}, (indexr x xs = None <-> indexr x ys = None)
                              /\ (forall {a}, indexr x xs = Some a -> exists b, P x xs a ys b).
...
Proof.
  ...
  - (* t_abs *)
    ...
    intros vx vxinT1.
    assert (HextG : 𝒞𝓉𝓍 (T1 :: Γ) ((mkD (val_type T1 Val ρ) (val_type T1 Typ ρ)) :: ρ)). {
      constructor. assumption.
    }
    assert (Hextg : ℰ𝓃𝓋 ((mkD (val_type T1 Val ρ) (val_type T1 Typ ρ)) :: ρ) (vx :: γ)). {
